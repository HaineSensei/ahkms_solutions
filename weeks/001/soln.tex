...
\]
Note the $r$-th term strictly exceeds the $(r-1)$-th term, since their difference is $1 + n_r \geq 1$; hence the set has exactly $k$ elements and the map indeed lands in $\calC_k$.
...
\[
    (m,n) \mapsto n + \sum_{i=1}^{m+n} i.
\]
One can think of this function as ordering $\N^2$ primarily by the sum of components, with a fall back to the second component (see figure \ref{Cantor}).
\begin{figure}[ht]
...
Then let $t$ be the $w$th triangle number. Then we know that $n = x - t$ and $m = w - n$.
...
To prove this by induction, the base case $k = 1$ is trivial. For the inductive step, we assume $|\N^k| = |\N|$.

\bigskip
That is, we have a bijection $\N^k \overset{f_k}\longleftrightarrow \N$. Then we can compose bijections as follows, to obtain a bijection $\N^{k+1} \overset{f_{k+1}}\longleftrightarrow \N$:
\[
